...
public class JsonHelperTest {
    ...
    public void testCastTweetFromString() throws IOException {
        ...
        ObjectMapper objectMapper = TwitterClient.OBJECT_MAPPER;
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        TweetDTOv1 result = objectMapper.readValue(tweet, TweetDTOv1.class);
        assertNotNull(result);
        assertNotNull(result.getLang());
        assertNotNull(result.getId());
        assertNotNull(result.getUser());
        assertNotNull(result.getCreatedAt());
        assertNotNull(result.getLikeCount());
    ...
    public void testParseTwitterFile() throws IOException, ParseException {
        ...
        assertNotNull(file);
        TweetDataDTO[] tweets = TwitterClient.OBJECT_MAPPER.readValue(file, TweetDataDTO[].class);
        ...
        for(TweetDataDTO tweetDataDTO : tweets){
            ...
            if(userId!=null){
                tweetDate = tweetDataDTO.getTweet().getCreatedAt();
    ...
    public void testGetListArray() throws IOException {
        ...
        JsonNode tweetList = TwitterClient.OBJECT_MAPPER.readValue(tweetFile1, JsonNode.class);
        List<ITweet> tweets = Arrays.asList(TwitterClient.OBJECT_MAPPER.treeToValue(tweetList, TweetDTOv1[].class));
        assertNotNull(tweets);
